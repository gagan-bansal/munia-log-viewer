...
function parseTemplate (tpl) {
  ...
  const format = tpl.replace(/{(.*?)}/g, (x, arg) => {
    ...
    parsedArgs = minimist(parseArgsStringToArgv(args), {
      string: ['f', 'l'],
      default: {'c': false},
      alias: {
        c: 'color',
    ...
    if (parsedArgs.include) parsedArgs.include = parsedArgs.include.split(',')
    if (parsedArgs.exclude) parsedArgs.exclude = parsedArgs.exclude.split(',')
    if (parsedArgs.colors) parsedArgs.colors = parsedArgs.colors.split(',')
      .reduce( (grp, pair) => { 
...
function setColorFunc (options) {
  ld.forEach(options.keys, (args, key) => {
    if (!args.color) {
      if (key === 'REST') {
          debugger
        let keyNames = ld.remove(ld.keys(options.keys), val => val !== 'REST')
        args.renderColor = (str, json) => {
          debugger
          return JSON.stringify(ld.omit(json, keyNames))
        }
      } else {
        args.renderColor = (str) => str;
      }
    } else {
      if (args.values) {
        ...
        args.renderColor = (str) => {
          return valuesColorMap[str] ? valuesColorMap[str](str) : str
        }
      } else if (key === options['level-key']) {
        args.renderColor = (str) => {
          return defaultLevelsColors[str] ? defaultLevelsColors[str](str) : str
        }
      } else {
        args.renderColor = (str) => {
